...
function InsideCard({ level }) {
  ...
  const handleClick = () => {
    if (!level.lock) {
      console.log(`Entering level ${level.id}`); // Log the level ID
      // Additional logic to handle level navigation or completion
    }
  };

  const levelContent = (
    <div className="LevelClick" onClick={handleClick}>
      <br />
...
//       // Send the "RequestLevelId" message to the window with the level ID
//       window.postMessage({ messageType: "RequestLevelId", levelId: level.id }, "*");
//     }
//   };
